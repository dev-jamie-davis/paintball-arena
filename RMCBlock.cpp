...
// Called every frame
void ARMCBlock::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);

}

void ARMCBlock::PostActorCreated()
{
...
void ARMCBlock::GenerateBoxMesh()
{
	...
	// Call the createboxmesh method, which will generate a 50x50x50 unit cube that can be jumped on.
...
void ARMCBlock::CreateBoxMesh(FVector BoxRadius, TArray<FVector>& Vertices, TArray<int32>& Triangles, TArray<FVector>& Normals, TArray<FVector2D>& UVs, TArray<FRuntimeMeshTangent>& Tangents, TArray<FColor>& Colors)
{
	FVector BoxVerts[4];
	// Establish the box vertices. A standard cube has eight corners, each represented by a vertex
	...
	// Like the meshes created by the RMC, the procedural mesh component objects also have
...
void ARMCBlock::PostInitializeComponents()
{
