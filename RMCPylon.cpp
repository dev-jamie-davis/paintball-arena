...
// Called every frame
void ARMCPylon::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);

}

void ARMCPylon::PostActorCreated()
{
...
void ARMCPylon::GenerateBoxMesh()
{
	...
	// Call the createboxmesh method, which will generate an upright rectangular pylon that's 120 units tall, and 50 units wide on any side.
	CreateBoxMesh(FVector(50, 50, 120), Vertices, Triangles, Normals, TextureCoordinates, Tangents, Colors);
...
void ARMCPylon::CreateBoxMesh(FVector BoxRadius, TArray<FVector>& Vertices, TArray<int32>& Triangles, TArray<FVector>& Normals, TArray<FVector2D>& UVs, TArray<FRuntimeMeshTangent>& Tangents, TArray<FColor>& Colors)
{
	FVector BoxVerts[4];
	// Establish the box vertices. A standard box has eight corners, each represented by a vertex
	BoxVerts[0] = FVector(-BoxRadius.X - FMath::RandRange(-25, 25), BoxRadius.Y, BoxRadius.Z);
	BoxVerts[1] = FVector(BoxRadius.X - FMath::RandRange(-25, 25), BoxRadius.Y, BoxRadius.Z);
	BoxVerts[2] = FVector(BoxRadius.X - FMath::RandRange(-25, 25), -BoxRadius.Y, BoxRadius.Z);
	BoxVerts[3] = FVector(-BoxRadius.X - FMath::RandRange(-25, 25), -BoxRadius.Y, BoxRadius.Z);
	BoxVerts[4] = FVector(-BoxRadius.X - FMath::RandRange(-25, 25), BoxRadius.Y, -BoxRadius.Z);
	BoxVerts[5] = FVector(BoxRadius.X - FMath::RandRange(-25, 25), BoxRadius.Y, -BoxRadius.Z);
	BoxVerts[6] = FVector(BoxRadius.X - FMath::RandRange(-25, 25), -BoxRadius.Y, -BoxRadius.Z);
	BoxVerts[7] = FVector(-BoxRadius.X - FMath::RandRange(-25, 25), -BoxRadius.Y, -BoxRadius.Z);
	Triangles.Reset();
...
void ARMCPylon::PostInitializeComponents()
{
